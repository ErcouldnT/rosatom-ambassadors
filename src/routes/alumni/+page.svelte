<script lang="ts">
	...
	import { ArrowRight, Search, X, GraduationCap } from '@lucide/svelte';
	...
	let t = $derived(translations[$language].alumniPage);
	function filteredAlumni(list: Ambassador[]) {
...
<div class="min-h-screen bg-base-100 pt-24 pb-20">
	<div class="container mx-auto px-6">
		...
		<div class="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
			{#await data.streamed.alumni}
				<div class="col-span-full flex justify-center py-20">
					<span class="loading loading-lg loading-spinner text-primary"></span>
				</div>
			{:then alumni}
				{#if filteredAlumni(alumni).length > 0}
					{#each filteredAlumni(alumni) as ambassador (ambassador.id)}
						<a
						...
						>
							...
							<div class="absolute top-4 right-4">
								<div class="badge gap-1 border-none bg-info/90 text-info-content backdrop-blur-sm">
			...
			{:catch error}
				<div class="col-span-full py-20 text-center text-error">
					<p>Error loading alumni: {error.message}</p>
				</div>
			{/await}
		</div>
	</div>
